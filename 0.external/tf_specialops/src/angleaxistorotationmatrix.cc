...
public:
  ...
  void Compute( OpKernelContext* context ) override 
  {
    ...
    if( GPU )
    {
      ...
      angleaxis_to_rotmatrix_gpu(device.stream(), output.data(), input.data(), num, false);
    }
    else
    {
      T* out = output.data();
      ...
      for( int64_t i = 0; i < num; ++i )
      {
        ...
        R = convert_to_rotation_matrix(in+3*i, ANGLEAXIS3).transpose(); // transpose because we want row major
      }
    }
...
REGISTER_KERNEL_BUILDER(                                                      \
    ...
    AngleAxisToRotationMatrixOp<false,type>);                                  
REG_KB(float)
REG_KB(double)
#undef REG_KB
...
REGISTER_KERNEL_BUILDER(                                                      \
    ...
    .Device(DEVICE_GPU)                                                       \
    .TypeConstraint<type>("T"),                                               \
    AngleAxisToRotationMatrixOp<true,type>);                                   
REG_KB(float)
REG_KB(double)
#undef REG_KB
...
REGISTER_KERNEL_BUILDER(                                                      \
    ...
    .Device(DEVICE_CPU)                                                       \
    .TypeConstraint<type>("T"),                                               \
    AngleAxisToRotationMatrixGradOp<false,type>);                              
REG_KB(float)
REG_KB(double)
#undef REG_KB
...
REGISTER_KERNEL_BUILDER(                                                      \
    ...
    .Device(DEVICE_GPU)                                                       \
    .TypeConstraint<type>("T"),                                               \
    AngleAxisToRotationMatrixGradOp<true,type>);                              
//REG_KB(float)
//REG_KB(double)
#undef REG_KB

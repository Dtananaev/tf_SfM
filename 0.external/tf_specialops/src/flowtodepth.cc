...
REGISTER_OP("FlowToDepth")
  ...
  .Doc(R"doc(
This op computes the depth from optical flow and the camera motion

The layer assumes that the internal camera parameters are the same for both
images that have been used to compute the flow.

Returns a tensor with the depth for the first image.
The format of the output tensor is NCHW with C=1; [batch, 1, height, width].


flow: 
  optical flow normalized or unnormalized

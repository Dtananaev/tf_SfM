...
REGISTER_OP("DepthToFlow")
  ...
  .Doc(R"doc(
This op computes the optical flow for an image pair based on the depth map
of the first image and the camera parameters of the second image.

The op assumes that the internal camera parameters is the same for both
cameras.
...
output:= A tensor with the optical flow from the first to the second image.
